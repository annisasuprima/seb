...
<body>
	...
	<div class="content">
		<div class="container">
		  ...
		  <div class="field">
            <td><label for="no_urut" class="label">No Urut</label></td>
            <td><input class="input" type="text" name="no_urut" id="no_urut" required></td>
          </div> 

          <div class="field">
            <td><label for="tgl_bencana" class="label">Tanggal Bencana</label></td>
            <td><input class="input" type="date" name="tgl_bencana" id="tgl_bencana"></td>
          </div>

          <div class="field">
            <td><label for="jenis_bencana" class="label">Jenis Bencana</label></td>
            <td>
            <select class="input" name="jenis_bencana" id="jenis_bencana">
				  <option value="tsunami">Tsunami</option>
				  <option value = "gempabumi">Gempa Bumi</option>
          <option value = "banjir">Banjir</option>
          <option value = "tanahlongsor">Tanah Longsor</option>
          <option value = "kebakaranhutan">Kebakaran Hutan</option>
          ...
          <div class="field">
            <td><label for="keterangan_tempat" class="label">Keterangan Tempat</label></td>
            <td>
            <select class="input" name="keterangan_tempat" id="keterangan_tempat" onchange="mapChange(this)">
...
<?php
  ...
  <script>
    ...
    function mapChange (event) {
      ...
      // const q = L.marker([bencana.lat, bencana.long]).addTo(mymap2);        

        // const ff = arr.filter(data => data.locate !== bencana.name)
        arr.map(item => {
          item.map.addTo(mymap2)
        ...
        var mymap2 = L.map('mapid2').setView([-0.9345808, 100.2511816], 10);
...
var jsonTest = new L.GeoJSON.AJAX(["/bencana/assets/map.geojson"],{onEachFeature:popUp, style: myStyle}).addTo(mymap2);
